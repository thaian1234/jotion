...
export function Heading() {
	...
	return (
		<header className="max-w-3xl space-y-4">
			...
			<h3 className="scroll-m-20 text-base sm:text-xl md:text-2xl font-medium tracking-tight">
				Jotion is the connected workspace where <br /> better, faster
				work happens
			</h3>
			{isLoading && !isAuthenticated && (
				<Spinner className="mx-auto" size="icon" />
			)}

			{isAuthenticated && !isLoading && (
				<Button asChild className="font-semibold animate-bounce">
					<Link href="/documents">
						Enter Jotion
